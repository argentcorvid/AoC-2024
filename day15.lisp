                 ...
                 (neighbor-fn (itm)
                   ...
                   (unless (or (wallp itm)
                     ...
                     (if (typep itm 'big-crate)
                         (list (next-in-dir itm) (find-other-half itm))
                         (list (next-in-dir itm)))))
                 (find-other-half (itm)
                   (find (+ (posn itm) (if (typep itm 'big-crate-l)
                                           1
                                           -1))
                         warehouse :key #'posn))
                 (movable? (itm)
                   (typecase itm
                     (big-crate
                      (let* ((other (find-other-half itm))
                             (next (next-in-dir itm))
                             (others-next (next-in-dir other)))
                        (cond ((and (typep next 'grid-floor)
                                    (typep others-next 'grid-floor))
                               t)
                              ((or (typep next 'grid-wall)
                                   (typep others-next 'grid-wall))
                               nil)
                              (t (and (movable? next)
                                      (movable? others-next))))))
                     (t nil)))
...
(defun test (&rest parts)
  (dolist (ip (list ;; *small-test-input* *p2-small-test-input*
                    *big-test-input*))
    (multiple-value-call #'run parts
